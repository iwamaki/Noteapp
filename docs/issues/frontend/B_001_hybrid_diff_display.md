...
- [ ] 行全体が追加・削除された場合、従来通り行単位で表示される
- [ ] 行内の一部が変更された場合、文字レベルのハイライト表示がされる
- [ ] 実験テストデータで以下が正しく表示される：
  - [ ] "これは削除される行" → 1行全体が削除として表示
  - [ ] "これは追加される行" → 1行全体が追加として表示
  - [ ] "これは変更されない行" → 1行として共通表示
  - [ ] これは追加される"" → これは追加される"文字" の場合、"文字"部分のみハイライト
  - [ ] これは削除される"文字" → これは削除される"" の場合、"文字"部分のみハイライト
  - [ ] これは変更される"文字" → これは変更される"学業" の場合、"文字"削除と"学業"追加が識別可能
- [ ] 行番号が正確に表示される（元テキスト・新テキスト両方）
- [ ] 型チェック（`npm run type-check`）が通る
- [ ] 既存の`validateDataConsistency`テストが通る
...
**現在の状況:**
- 文字ベース差分実装により、差分表示が断片化する問題が顕在化
- 行ベース版のバックアップは正常動作を確認済み
- `DiffLine`インターフェースと`DiffViewer`コンポーネントは文字ベース版に対応済み

**次のアクション:**

1. **Phase 1: データ構造の拡張**
   - `DiffLine`に`inlineChanges?: InlineChange[]`フィールドを追加
   - `InlineChange`インターフェースを定義

2. **Phase 2: ハイブリッド差分計算ロジックの実装**
   - `generateDiff()`を以下の手順で書き換え：
     a. `calculateLineDiff()`で行レベル差分を取得
     b. 連続する削除・追加行を変更ブロックとして検出
     c. 変更ブロック内の行ペアに対して`calculateCharDiff()`を適用
     d. 結果を`inlineChanges`フィールドに格納
   - `calculateCharDiff()`は既存実装をそのまま活用可能

3. **Phase 3: UI表示ロジックの実装**
   - `DiffViewer.tsx`の`renderDiffLine()`を拡張
   - `inlineChanges`が存在する場合、ネストした`<Text>`コンポーネントでハイライト表示
   - インラインスタイルを定義（削除：赤背景、追加：緑背景）

4. **Phase 4: テストと検証**
   - 実験テストデータで受け入れ条件を満たすか確認
   - `npm run type-check`実行
   - 必要に応じて`validateDataConsistency`を調整

**考慮事項/ヒント:**
- 行ベース版（`diffService.line-based.backup.ts:132-183`）の変更ブロック検出ロジックを参考にできる
- 文字ベース差分計算（`diffService.ts:62-98`の`calculateCharDiff`）は既に実装済みで動作確認済み
- 変更ブロック内で削除行数と追加行数が異なる場合の対応戦略を検討すること（例: 行数が少ない方に合わせてペアリング、残りは行全体として表示）
