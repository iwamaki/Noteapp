"""
Infrastructure Layer - WebSocket Connection Manager

WebSocket接続を管理し、バックエンドとフロントエンド間の
双方向通信を実現します。

Note:
    この実装は旧 src/api/websocket.py のConnectionManagerを
    新しいアーキテクチャに移行したものです。
...
class WebSocketConnectionManager:
    ...
    async def connect(self, websocket: WebSocket, client_id: str):
        ...
        logger.info({"event": "websocket_connected", "client_id": client_id})
    ...
    def disconnect(self, client_id: str):
        ...
        logger.info({"event": "websocket_disconnected", "client_id": client_id})
    ...
    def resolve_request(self, request_id: str, content: str | None, error: str | None = None):
        ...
        if request_id not in self.pending_requests:
            logger.warning({"event": "unknown_request_id", "request_id": request_id})
            return
        ...
        if not future.done():
            if error:
                ...
                logger.debug({"event": "request_resolved", "request_id": request_id})
    ...
    def handle_ping(self, client_id: str):
        ...
        if client_id in self.active_connections:
            # 最後のping受信時刻を更新
            self.last_ping[client_id] = time.time()
            logger.debug({"event": "ping_received", "client_id": client_id})
    ...
    async def check_stale_connections(self):
        ...
        """
        logger.info({"event": "stale_check_task_started"})
            ...
            try:
                ...
                for client_id, last_time in list(self.last_ping.items()):
                    if now - last_time > 60:  # 60秒以上pingがない
                        ...
                        logger.warning({
                            "event": "stale_connection_detected",
                            "client_id": client_id,
                            "last_ping_seconds_ago": round(now - last_time, 1)
                        })
                ...
                for client_id in stale_clients:
                    if client_id in self.active_connections:
                        try:
                            ...
                            logger.info({
                                "event": "stale_connection_closed",
                                "client_id": client_id
                            })
                        except Exception as e:
                            logger.error({
                                "event": "error_closing_stale_connection",
                                "client_id": client_id,
                                "error": str(e)
                            })
                        finally:
                ...
                logger.info({"event": "stale_check_task_cancelled"})
                break
            except Exception as e:
                logger.error({
                    "event": "stale_check_error",
                    "error": str(e)
                })
...
def get_websocket_manager() -> WebSocketConnectionManager:
    """
    WebSocketマネージャーのグローバルインスタンスを取得

    Note:
        将来的にはDIに移行する予定です。
        現在は後方互換性のために提供されています。

    Returns:
        WebSocketConnectionManager: マネージャーインスタンス
    """
