"""
File Operations Tool - Read File

指定されたファイルの内容を読み取るツール（新アーキテクチャ版）

Note:
    このツールは新システム（main_new.py）で使用されます。
    旧システムのツール（src/llm/tools/read_file.py）はそのまま保持されます。
"""
...
async def read_file(title: str) -> str:
    ...
    """
    logger.info({"event": "read_file_called", "title": title})
    ...
    if current_file_context:
        ...
        if title.strip() == current_filename.strip():
            logger.info({"event": "file_found_in_context", "title": title})
            if current_content:
    ...
    if not file_exists:
        # ファイルが見つからない
        available_files = [
            f.get("title", "") for f in all_files if f.get("type") == "file"
        ]
        return (
            f"エラー: ファイル '{title}' が見つかりませんでした。\n\n利用可能なファイル:\n"
    ...
    if not client_id:
        logger.error({"event": "no_client_id", "title": title})
        return f"エラー: WebSocket接続が確立されていません。ファイル '{title}' を読み取れません。アプリを再起動してください。"

    try:
        logger.info({
            "event": "requesting_file_content",
            "title": title,
            "client_id": client_id,
        })

        # 新しいWebSocketマネージャーを使用
        websocket_manager = get_websocket_manager()
        content = await websocket_manager.request_file_content(
            client_id, title, timeout=30
        )
        ...
        error_msg = str(e)
        logger.error({
            "event": "file_content_request_failed",
            "title": title,
            "error": error_msg,
        })
