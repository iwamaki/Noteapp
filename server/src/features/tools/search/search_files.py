...
ファイルを検索するツール（新アーキテクチャ版）

Note:
    このツールは新システム（main_new.py）で使用されます。
    旧システムのツール（src/llm/tools/search_files.py）はそのまま保持されます。
"""
...
async def search_files(
    ...
    """
    logger.info({"event": "search_files_called", "query": query, "search_type": search_type})

    # WebSocket接続のクライアントIDを取得
    client_id = get_client_id()
    if not client_id:
        logger.error({"event": "no_client_id", "query": query})
        return "エラー: WebSocket接続が確立されていません。検索を実行できません。アプリを再起動してください。"

    try:
        logger.info({
            "event": "requesting_search",
            "query": query,
        ...
        results = await websocket_manager.request_search_results(
            client_id=client_id,
            query=query,
            search_type=search_type,
            timeout=30,
        ...
        # 結果を整形
        result_parts = [
            f"検索結果: クエリ '{query}' ({search_type}検索) で {len(results)}件のファイルが見つかりました。\n"
        ]
