...
class CommandExtractorService:
    ...
    def _handle_edit_file(self, tool_input: dict[str, Any]) -> LLMCommand | None:
        ...
        # Clean up unwanted markdown syntax generated by LLM
        if content:
            # Remove markdown code blocks
            content = content.replace('```markdown\n', '').replace('\n```', '').replace('```\n', '')
            # Remove leading/trailing "---" lines (markdown separators/frontmatter)
            lines = content.split('\n')
            # Remove leading "---" only lines
            while lines and lines[0].strip() == '---':
                lines.pop(0)
            # Remove trailing "---" only lines
